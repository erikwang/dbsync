...
public class CompareData{
				...
				LogMod log;
				String _emptyChar; //如果dvl or StrArr中为空的位置，由什么字符代替空
...
//------------------------------------------------------------------------------------		
		public boolean initSystem(){  //清空sis.sqlcommand
				boolean flag = false;
				try{
...
//------------------------------------------------------------------------------------		
		public void compareDataVectorList(DataVectorList sdvl,DataVectorList ddvl){
				...
				System.out.println("[CD S to D] >Begin a Source to Destination compare"); //源到目标的比较，可判定整块追加的情况，同时平行比较由此发动
				...
				System.out.println("[CD D to S] >Begin a Destination to Source compare");	//目标到源的比较，判定整块删除的情况
			...
			try{
				...
				for(int y = 1;y < (numberOfColumns+1);y++){
						...
						if ( ! (dtcolumntype.equals("ROWID"))){
								//if (! ((dtcolumntype.equals("DATE")) || (dtcolumntype.equals("LONG")) || (dtcolumntype.equals("NUMBER")) || (dtcolumntype.equals("BLOB")) ||  (dtcolumntype.equals("CLOB")))){ 如果希望能自动判断data类型等，请在这里继续编码，增加记录data类型的index数组，以便在生成insert语句中对应位置使用to_date()
	...
	//------------------------------------------------------------------------------------			
		public String getMd5(Data tempdata , int offset){ //offset表示从第几列开始取数据，保证追加的字段在表的最前n列 返回MD5值
				int tsize = tempdata.getSize();
				String output;
				output = "";
...
//------------------------------------------------------------------------------------			
		public String getMd5OriStr(Data tempdata , int offset){ //offset表示从第几列开始取数据，保证追加的字段在表的最前n列 返回String
				int tsize = tempdata.getSize();
				String output;
				output = "";
				for (int j = offset - 1;j<tsize; j++){
...
//------------------------------------------------------------------------------------			
		public long getMd5OriLong(Data tempdata , int offset){ //offset表示从第几列开始取数据，保证追加的字段在表的最前n列 返回String
				int tsize = tempdata.getSize();
				long  output;
				output = 0;
				for (int j = offset - 1;j<tsize; j++){
...
//------------------------------------------------------------------------------------			
		public void executeSqlCommand(){ // 执行sis.sqlcommand表中所有的sql语句
			//sql0 用于置sql执行完成后的状态
			//sql1 选取sqlcommand表中所有尚未执行的sql语句
			//sql2 执行sql语句的载体
...
//------------------------------------------------------------------------------------			
		public void paraCompareDv(DataVector sdv,DataVector ddv){ //平行比较
				boolean sqlflag;
				String[][] sstrarr;
				String[][] dstrarr;
	...
	//------------------------------------------------------------------------------------
	public boolean paraCompareStrArr(String[][] sstrarr,String[][] dstrarr,DataVector sdv){ //平行比较String Array
		try{
				String dTable = xmlReader.getFromConf("DTABLE",confFile); 		//为了确定表中的列名，以此表为列的标准
				String sTable = xmlReader.getFromConf("STABLE",confFile);
				boolean sqlflag;
				sqlflag = false;
				Data tData = new Data();						//用于记录一条纪录的信息
				SfToDec sftd;
				sftd = new SfToDec();
				String sql1;
				sql1 = "";
				...
				boolean lastDel;
				lastDel = false;
				for(int t = 0 ;t<(sstrarr.length);t++){		//平行比较次数
					if(!(sstrarr[t][1].equals(dstrarr[t][1]))){	//发现两边数组出现值不同的情况
							//System.out.println(sstrarr[t][1]+" |||| "+dstrarr[t][1]);
							sqlflag = false;
							//if((sstrarr[t][0].equals("*")) && ( ! dstrarr[t][0].equals("*"))){ //源不存在记录，目标存在，认为源发生了数据删除
							if((sstrarr[t][0].equals(_emptyChar)) && ( ! dstrarr[t][0].equals(_emptyChar))){ //源不存在记录，目标存在，认为源发生了数据删除
							...
							if(! (sstrarr[t][0].equals(_emptyChar)) && ( (dstrarr[t][0].equals(_emptyChar)))){ //源存在记录，目标不存在，认为源发生了数据增加
							...
							if(! (sstrarr[t][0].equals(_emptyChar)) && ( !(dstrarr[t][0].equals(_emptyChar)))){ //源目标均存在记录，认为发生了源数据变更
									//if (lastDel == true){i++;lastDel = false;}
...
//------------------------------------------------------------------------------------
	  public String[][] getStringArray(DataVector dv1,DataVector dv2,int offset){ //从两个dv决定String Array的大小，并且将dv1的数据填写到String Array中
	  	//offset 表示在dv中的列数，例如d_table追加了3个列,那么为了和s_table中列的数量一致,在此表明列偏移量
	  	SfToDec sftd;
			sftd = new SfToDec();
